/*
 * Modified entities.js to fix player sprite loading.
 *
 * The original game code expected player images to reside directly under
 * the `assets/characters` folder with names like `main_walking_down.png`
 * or `main_stopped.png`. However, the actual sprite assets live inside
 * a `main` subfolder (e.g., `assets/characters/main/main_walking_down.png`).
 * This mismatch caused the player character to render as a plain colored
 * rectangle that blended with the environment. To resolve this, the
 * Player.draw method now points to the correct subfolder and file names.
 */
...
export class Player extends Sprite {
  ...
  draw(ctx) {
    // Determine if the player is moving. Use a small threshold to avoid
    // treating tiny velocity values as movement (helps with floating-point noise).
    const moving = Math.abs(this.vx) + Math.abs(this.vy) > 0.1;
    // Build the asset key. Player sprites live in the `main` subfolder of
    // `assets/characters`. When moving, the filenames follow the pattern
    // `main_walking_<direction>.png`; when idle, the file is
    // `main_stopped.png`.
    let filename;
    if (moving) {
      filename = `main_walking_${this.facing}.png`;
    } else {
      filename = `main_stopped.png`;
    }
    const key = `characters/main/${filename}`;
    this.imgKey = key;
    super.draw(ctx);
  }
}
...
export class Projectile extends Sprite {
  constructor(engine, opts) {
    super(engine, Object.assign({ w: 12, h: 12, collides: false }, opts));
    this.lifetime = 2000;
